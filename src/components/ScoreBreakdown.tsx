...
export function ScoreBreakdown({ level, elapsedTime, shuffles }: ScoreBreakdownProps) {
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-2 text-sm">
      <h3 className="font-semibold text-gray-900 mb-1.5">Score Breakdown</h3>
      ...
      <div className="space-y-1">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-1 min-w-0">
            ...
            <span className="text-gray-600 truncate">Level {level}</span>
          </div>
          <span className="font-medium text-gray-900 flex-none">+{levelScore.toLocaleString()}</span>
        </div>

        <div className="flex justify-between items-center">
          <div className="flex items-center gap-1 min-w-0">
            <Clock className="w-4 h-4 text-green-500 flex-none" />
            <span className="text-gray-600 truncate">Time ({Math.round(timeRatio * 100)}%)</span>
          </div>
          <span className="font-medium text-gray-900 flex-none">+{timeScore.toLocaleString()}</span>
        </div>

        <div className="flex justify-between items-center">
          <div className="flex items-center gap-1 min-w-0">
            <Ban className="w-4 h-4 text-red-500 flex-none" />
            <span className="text-gray-600 truncate">Shuffles (×{shuffles})</span>
          </div>
        ...
        <div className="pt-1 mt-1 border-t border-gray-200">
          <div className="flex justify-between items-center">
            <span className="font-semibold text-gray-900 truncate">Round Total</span>
            <span className="font-bold text-blue-600 flex-none">{totalScore.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
